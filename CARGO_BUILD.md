...
- **Static libelf / zlib on device:** [`scheds/rust/scx_lavd/Cargo.toml`](scheds/rust/scx_lavd/Cargo.toml) enables **`libbpf-rs`** feature **`static`** for `target_os = "android"`, so **`libbpf-sys`** links **`libelf`** and **`zlib`** as **static** archives from your prefix; the **`scx_lavd`** binary does **not** need **`libelf.so.1`** on the device. The workspace uses a small **[`libbpf-sys` fork](rust/libbpf_sys_scx)** (via **`[patch.crates-io]`** in the root [`Cargo.toml`](Cargo.toml)) so `LIBBPF_SYS_LIBRARY_PATH` is applied **before** `-l` flags and so **glibc Linux host** builds pick up **`libzstd`** when distro **`libelf.a`** references ZSTD (install **`libelf-dev`** and **`libzstd-dev`** on Debian/Ubuntu). **`build_scx_lavd.sh`** auto-sets **`LIBBPF_SYS_LIBRARY_PATH_<host>`** on Linux when **`libelf.a`** is under **`/usr/lib/<arch>-linux-gnu`**.
...
If you maintain your own sysroot instead of the bootstrap script, keep **`libbpf-rs` default features** (do **not** enable feature **`vendored`**: vendored elfutils expects GNU **argp** and fails under Bionic). Export **`SCX_ANDROID_LIBELF_PREFIX`** and use **`build_scx_lavd.sh`**, which maps it to **`LIBBPF_SYS_EXTRA_CFLAGS`** and **`LIBBPF_SYS_LIBRARY_PATH_aarch64_linux_android`**. The workspace **`libbpf-sys`** patch applies to all crates that depend on it (same version as crates.io).
