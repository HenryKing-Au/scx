...
fn main() {
    ...
    // Emit extra library search paths *before* -l flags so rustc can resolve
    // `static=` archives (upstream libbpf-sys printed them after -l, which breaks
    // some cross-builds). See sched-ext/scx Android notes.
    ...
    // When libbpf-rs enables `static`, Cargo can apply static-libelf to the host
    // libbpf-sys built for build-dependencies. Many distro libelf.a archives pull
    // in ZSTD compression helpers — link libzstd for typical glibc Linux hosts
    // (not Android targets). Android libelf from scx bootstrap has no ZSTD.
    if static_libelf {
        let target = env::var("TARGET").unwrap_or_default();
        if target.contains("linux") && target.contains("gnu") && !target.contains("android") {
